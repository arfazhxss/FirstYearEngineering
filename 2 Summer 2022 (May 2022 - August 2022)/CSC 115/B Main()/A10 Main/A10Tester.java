...
public class A10Tester {
    ...
    public static void hashmapEntryTest2() {
        ...
        for(int i=0; i<hash2_size; i++) {
            System.out.println("(" + hash2_keys_order[i] + "," + hash2_values_order[i] + ")");
        }
    ...
    public static void map3Test() {
        System.out.println("**** begin map stress test with random keys:values");
        Map<Integer, Integer> d = A10Tester.<Integer,Integer>createMap();
        Map<Integer, Integer> lMap = new LinkedMap<Integer,Integer>();
        int testSize = 10000;
        ...
        displayResults(d.size() == lMap.size(), "put + size");

        boolean passed = true;
        List<Entry<Integer, Integer>> lMapEntries = lMap.entryList();
        Iterator<Entry<Integer, Integer>> iter = lMapEntries.iterator();

        while (iter.hasNext() && passed) {
            try {
        ...
        displayResults(passed, "get");
        System.out.println("****** end map stress test with random keys:values\n");
    }
