...
void generateSVGfile(Matrix wireFrame[], int noEdges) {
	...
	computeTransformationMatrix(M, &Mrows, &Mcols, 150, 375, 0, 125, 100, 100, 0);
	drawWireFrame(outFile, wireFrame, noEdges, M, OBJECT_COLOR_1);
	computeTransformationMatrix(M, &Mrows, &Mcols, 100, 125, 0, 375, 100, 0, 0);
	drawWireFrame(outFile, wireFrame, noEdges, M, OBJECT_COLOR_2);
	computeTransformationMatrix(M, &Mrows, &Mcols, 50, 375, 0, 375, 100, -100, 0);
